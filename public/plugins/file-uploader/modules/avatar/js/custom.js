  ...
  $('input[name="profile_img"]').fileuploader({
    ...
    thumbnails: {
      ...
      onItemShow: function (item) {
        if (item.choosed) item.html.addClass("is-image-waiting");
      },
      onImageLoaded: function (item, listEl, parentEl, newInputEl, inputEl) {
        if (item.choosed && !item.isSaving) {
          if (
            item.reader.node &&
            item.reader.width >= 256 &&
            item.reader.height >= 256
          ) {
            item.image.hide();
            item.popup.open();
            item.editor.cropper();
        ...
        } else if (item.data.isDefault) item.html.addClass("is-default");
    ...
    editor: {
      ...
      onSave: function (base64, item, listEl, parentEl, newInputEl, inputEl) {
        ...
        if (item.upload) {
          if (
            api.getFiles().length == 2 &&
            (api.getFiles()[0].data.isDefault || api.getFiles()[0].upload)
          )
            api.getFiles()[0].remove();
          parentEl.find(".fileuploader-menu ul a").show();

          if (item.upload.send) return item.upload.send();
          if (item.upload.resend) return item.upload.resend();
    ...
    upload: {
      ...
      onSuccess: function (
        ...
        $progressBar
          .find("span")
          .html('<i class="fileuploader-icon-success"></i>');
        parentEl.find('[data-action="fileuploader-edit"]').show();
        setTimeout(function () {
      ...
      onError: function (item, listEl, parentEl, newInputEl, inputEl) {
        ...
        if (item.upload.status != "cancelled")
          $progressBar
            .find("span")
            .attr("data-action", "fileuploader-retry")
            .html('<i class="fileuploader-icon-retry"></i>');
      },
      onProgress: function (data, item) {
        ...
        else if (data.percentage >= 99) data.percentage = 100;
        else $progressBar.removeClass("is-reset");
        if (!$progressBar.children().length)
          $progressBar.html(
            '<span></span><svg><circle class="progress-dash"></circle><circle class="progress-circle"></circle></svg>'
          );
    ...
    afterRender: function (listEl, parentEl, newInputEl, inputEl) {
      ...
      // set drop container
      api.getOptions().dragDrop.container = parentEl.find(
      ...
      parentEl.on("click", "[data-action]", function () {
        var $this = $(this),
          ...
          item = api.getFiles().length
            ? api.getFiles()[api.getFiles().length - 1]
            : null;
        ...
        switch (action) {
          ...
          case "fileuploader-edit":
            if (item && item.popup) {
              if (!$this.is(".fileuploader-action-popup")) item.popup.open();
              item.editor.cropper();
            }
            break;
          case "fileuploader-retry":
            if (item && item.upload.retry) item.upload.retry();
            break;
      ...
      $("body").on("click", function (e) {
        ...
        if (
          $target.is(".fileuploader-menu-open") ||
          $target.closest(".fileuploader-menu-open").length
        )
      ...
      if (defaultAvatar && !listEl.find("> .is-default").length)
        api.append({
          ...
          data: {
            isDefault: true,
            popup: false,
            listProps: { is_default: true },
          },
        });

      parentEl
        .find(".fileuploader-menu ul a")
        .hide()
        .filter('[data-action="fileuploader-input"]')
        .show();
    },
